...
type
  ...
  uc_x86_mmr = record
     selector : UInt16; // not used by GDTR and IDTR .
     base     : UInt64; // handle 32 or 64 bit CPUs .
     limit    : UInt32;
     flags    : UInt32; // not used by GDTR and IDTR .
  end;
...
type
  ...
  uc_cb_insn_syscall_t = procedure(uc : uc_engine; user_data : Pointer); cdecl;
...
(*
 ...
 for detailed error).
*)
  function uc_open(arch : uc_arch; mode : uc_mode; var uc : uc_engine) : uc_err; cdecl;
var
(*
...
(*
  ...
  @return UC_ERR_OK on success, or other value on failure (refer to uc_err enum \
    for detailed error).
*)
  function uc_context_restore(uc : uc_engine; context : uc_context) : uc_err; cdecl;
...
// this way when i use auto-complete it will act like a function not variable .
// will implement other functions later :D .
function uc_open(arch : uc_arch; mode : uc_mode; var uc : uc_engine) : uc_err; cdecl;
begin
   Result := uc_open_ref(arch,mode,uc);
end;
function uc_context_restore(uc : uc_engine; context : uc_context) : uc_err; cdecl;
begin
  Result := uc_context_restore_ref(uc,context);
end;
...
begin
  ...
  begin
    ...
    @uc_open_ref := dyn_loadfunc('uc_open');
    if (@uc_open_ref = nil) then exit(false);
    ...
    @uc_context_restore_ref := dyn_loadfunc('uc_context_restore');
    if (@uc_context_restore_ref = nil) then exit(false);
